...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X =nn.utils.rnn.pack_padded_sequence(X, source_lengths, batch_first=False, enforce_sorted=True)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X) 
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens = torch.permute(enc_hiddens, (1, 0, 2))

        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), 1) #shape will be (b, 2h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            decstate, combinedoutput, et= self.step(Ybar_t, dec_state,enc_hiddens, enc_hiddens_proj, enc_masks)
            dec_state = decstate
            combined_outputs.append(combinedoutput)
            o_prev = combinedoutput
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hid = torch.unsqueeze(dec_hidden, 2)
        e_t = torch.bmm(enc_hiddens_proj, dec_hid)
        ...
        alph = torch.unsqueeze(alpha_t, 1)
        a_t = torch.squeeze(torch.bmm(alph, enc_hiddens), 1)
